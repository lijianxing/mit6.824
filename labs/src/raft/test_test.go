...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("-------------------Disconnect current leader %d---------------", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	DPrintf("-------------------connect peer %d---------------", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	DPrintf("-------------------disconnect %d %d---------------", leader2, (leader2+1)%servers)
	...
	DPrintf("-------------------reconnect %d ---------------", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	DPrintf("-------------------reconnect %d ---------------", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
